...
-- `chili oil` sengaja bermargin nol (harga = modal). `saus keju` dibuat
-- bermargin supaya laporan benar-benar menguji pemisahan omzet extra dari
-- modal extra — dengan chili oil saja, dua kesalahan hitung bisa saling
-- meniadakan dan bug-nya tidak kelihatan.
...
-- --- Transaksi contoh ---------------------------------------
-- 60 nota tersebar ~27 hari ke belakang: cukup untuk menguji paginasi riwayat
-- (25 per halaman) sekaligus mengisi ketiga rentang filter laporan.
--
-- Semua angkanya diturunkan dari nomor urut, bukan `random()`, supaya dua kali
-- reset menghasilkan data yang sama persis dan hasil ujian bisa dibandingkan.
do $$
declare
  v_tx       uuid;
  v_item     uuid;
  ...
  v_nama     text;
begin
  -- Idempoten: kalau sudah ada transaksi, jangan menumpuk data ganda.
  if exists (select 1 from public.transaksi) then
    return;
  end if;

  for v_n in 1..60 loop
    -- Nomor terbesar = paling baru, dan nota terakhir berumur nol supaya
    -- jendela pembatalan 10 menit bisa langsung diuji kasir.
    ...
    -- Sepertiga bayar pas, sisanya dibulatkan ke atas per 10 ribu supaya ada
    -- kembalian yang bisa dilihat di riwayat.
