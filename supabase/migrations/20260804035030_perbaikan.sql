...
CREATE TABLE public.transaksi_item (id uuid DEFAULT gen_random_uuid() NOT NULL, transaksi_id uuid NOT NULL, variant_id uuid, nama_produk text NOT NULL, nama_varian text NOT NULL, jumlah_pcs smallint, harga_satuan numeric(12,2) NOT NULL, modal_bahan_satuan numeric(12,2) NOT NULL, modal_kemasan_satuan numeric(12,2) NOT NULL, hpp_satuan numeric(12,2) GENERATED ALWAYS AS ((modal_bahan_satuan + modal_kemasan_satuan)) STORED NOT NULL, qty smallint NOT NULL);
...
CREATE VIEW public.v_hpp_varian WITH (security_invoker=true) AS SELECT v.id,
    ...
    (v.modal_bahan + COALESCE(k.modal_kemasan, (0)::numeric)) AS modal_total,
    ((v.harga_jual - v.modal_bahan) - COALESCE(k.modal_kemasan, (0)::numeric)) AS margin,
